    ...
    fn build_from(&mut self, stmt: &Stmt) -> String {
        ...
        match stmt {
            ...
            If(exp, stmt, opt_stmt) => {
                let src = self.build_from(exp);
                let then_label = self.make_then_label();
                let else_label = self.make_else_label();
                let endif_label = self.make_endif_label();
